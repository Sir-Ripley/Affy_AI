...
class FirstOrdinalSerializer<T : Enum<T>>(private val enumClass: KClass<T>) : KSerializer<T> {
  ...
  // ⚡ Bolt: Cache Enum values, name mappings, and serialName mappings to avoid expensive reflection
  // and list iteration on every serialize/deserialize call.
  private val cachedValues by lazy { enumClass.enumValues() }

  private val nameToValue by lazy {
    val map = mutableMapOf<String, T>()
    for (value in cachedValues) {
      if (!map.containsKey(value.serialName)) {
  ...
  private val valueToName by lazy {
  ...
  override fun serialize(encoder: Encoder, value: T) {
    encoder.encodeString(valueToName[value] ?: value.name)
  }
}
