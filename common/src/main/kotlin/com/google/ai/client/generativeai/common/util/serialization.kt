...
class FirstOrdinalSerializer<T : Enum<T>>(private val enumClass: KClass<T>) : KSerializer<T> {
  ...
  // Cache the values and names to avoid reflection on every serialization/deserialization
  private val values = enumClass.enumValues()
  private val nameToValue = values.associateBy { it.serialName }
  private val valueToName = values.associateWith { it.serialName }
