...
class FirstOrdinalSerializer<T : Enum<T>>(private val enumClass: KClass<T>) : KSerializer<T> {
  ...
  private val cachedEnumValues: Array<T> by lazy { enumClass.enumValues() }

  private val valueToName: Map<T, String> by lazy {
    val map = mutableMapOf<T, String>()
    for (value in cachedEnumValues) {
  ...
  private val nameToValue: Map<String, T> by lazy {
    ...
    for (value in cachedEnumValues) {
      val key = valueToName[value] ?: value.serialName
  ...
  override fun deserialize(decoder: Decoder): T {
    ...
    return nameToValue[name] ?: cachedEnumValues.first().also { printWarning(name) }
  }
