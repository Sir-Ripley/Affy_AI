...
class FirstOrdinalSerializer<T : Enum<T>>(private val enumClass: KClass<T>) : KSerializer<T> {
  ...
  private val valueToName: Map<T, String> by lazy { cachedValues.associateWith { it.serialName } }

  private val firstValue: T by lazy { cachedValues.first() }

  override fun deserialize(decoder: Decoder): T {
    val name = decoder.decodeString()
    return nameToValue[name] ?: firstValue.also { printWarning(name) }
  }
  ...
  override fun serialize(encoder: Encoder, value: T) {
    val name = valueToName[value] ?: value.serialName
