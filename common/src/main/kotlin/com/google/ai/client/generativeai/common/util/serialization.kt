...
class FirstOrdinalSerializer<T : Enum<T>>(private val enumClass: KClass<T>) : KSerializer<T> {
  ...
  private val serialNames = cachedValues.associateWith { it.serialName }
  private val valuesBySerialName = serialNames.entries.associate { it.value to it.key }

  override fun deserialize(decoder: Decoder): T {
    val name = decoder.decodeString()

    return valuesBySerialName[name] ?: cachedValues.first().also { printWarning(name) }
  }
  ...
  override fun serialize(encoder: Encoder, value: T) {
    encoder.encodeString(serialNames[value] ?: value.serialName)
  }
}
