...
class FirstOrdinalSerializer<T : Enum<T>>(private val enumClass: KClass<T>) : KSerializer<T> {
  ...
  private val stringToEnum by lazy {
  ...
  private val enumToString by lazy {
  ...
  override fun deserialize(decoder: Decoder): T {
    ...
    return stringToEnum[name] ?: values.first().also { printWarning(name) }
  }
  ...
  override fun serialize(encoder: Encoder, value: T) {
    encoder.encodeString(enumToString[value] ?: value.serialName)
  }
}
